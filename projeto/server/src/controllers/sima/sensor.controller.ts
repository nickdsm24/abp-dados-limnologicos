...
// 2. Importa o Model
import { SensorModel } from "../../models/sima/sensor.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const { data: rawData, total } = await SensorModel.findPaginated({
        ...
        logger.error("Erro ao consultar tbsensor", {
...
export const getById = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (isNaN(idSensor)) {
            res.status(400).json({
                success: false,
                error: `ID ${req.params.idSensor} inválido.`,
            });
            return;
        }

        // 1. Pede o dado ao Model
        const rawData = await SensorModel.findById(idSensor);

        // 2. Verifica se foi encontrado
        if (!rawData) {
            res.status(404).json({
                success: false,
                error: `Registro de sensor não encontrado.`,
            });
            return;
        }
        ...
        logger.error(`Erro ao consultar registro por ID na tabela tbsensor: ${req.params.idSensor}`, {
            message: error.message,
            stack: error.stack,
        });
        res.status(500).json({
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (range === "page") {
            const { data } = await SensorModel.findPaginated({
                filters: filters || {},
                page: page || 1,
            ...
            // range === 'all'
            rawData = await SensorModel.findAll({
                filters: filters || {},
            });
        }
        ...
        const fileName = `export_sensor_${new Date().toISOString().slice(0, 10)}.${format}`;
        ...
        logger.error("Erro ao exportar dados de tbsensor", {
            message: error.message,
            stack: error.stack,
        });
        res.status(500).json({
