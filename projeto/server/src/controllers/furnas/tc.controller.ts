...
// 2. Importa o Model
import { TbtcModel } from "../../models/furnas/tc.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const { data: rawData, total } = await TbtcModel.findPaginated({
...
export const getById = async (req: Request, res: Response): Promise<void> => {
    try {
        const idTc = Number(req.params.id);
        ...
        const rawData = await TbtcModel.findById(idTc);

        // 2. Verifica se foi encontrado
        if (!rawData) {
            res.status(404).json({
                success: false,
                error: `Registro de TC (tbtc) não encontrado.`,
            });
            return;
        }
        ...
        logger.error(`Erro ao consultar tbtc por ID ${req.params.id}`, {
            message: error.message,
            stack: error.stack,
        });
        res.status(500).json({
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (range === "page") {
            const { data } = await TbtcModel.findPaginated({
                filters: filters || {},
                page: page || 1,
            ...
            // range === 'all'
            rawData = await TbtcModel.findAll({
                filters: filters || {},
            });
        }
        ...
        const fileName = `export_tbtc_${new Date().toISOString().slice(0, 10)}.${format}`;
        ...
        logger.error("Erro ao exportar dados de tbtc", {
            message: error.message,
            stack: error.stack,
        });
        res.status(500).json({
