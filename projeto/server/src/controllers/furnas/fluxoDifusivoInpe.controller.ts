...
import { FluxoDifusivoInpeModel } from "../../models/furnas/fluxoDifusivoInpe.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: rawData, total } =
      await FluxoDifusivoInpeModel.findPaginated({
        filters: req.query,
        page,
        limit,
      });
    ...
    logger.error(`Erro ao buscar dados de fluxo difusivo (INPE):`, {
...
export const getById = async (req: Request, res: Response): Promise<void> => {
  try {
    const idFDInpe = Number(req.params.idFluxoDifusivoInpe);

    if (isNaN(idFDInpe)) {
      res.status(400).json({
        success: false,
        error: `ID ${req.params.idFluxoDifusivoInpe} inválido.`,
      });
      return;
    }

    // 1. Pede o dado ao Model
    const rawData = await FluxoDifusivoInpeModel.findById(idFDInpe);

    // 2. Verifica se foi encontrado
    if (!rawData) {
      res.status(404).json({
        success: false,
        error: "Registro de dados de fluxo difusivo (INPE) não encontrado.",
      });
      return;
    }
    ...
    logger.error(
      `Erro ao buscar registro por ID ${req.params.idFluxoDifusivoInpe} na tabela de fluxo difusivo (INPE).`,
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // ✅ MUDANÇA AQUI: Usa FluxoDifusivoInpeModel
    if (range === 'page') {
      const { data } = await FluxoDifusivoInpeModel.findPaginated({
        filters: filters || {},
        page: page || 1,
      ...
      // range === 'all'
      rawData = await FluxoDifusivoInpeModel.findAll({
        filters: filters || {},
      });
    }
    ...
    const fileName = `export_fluxo_difusivo_inpe_${new Date().toISOString().slice(0, 10)}.${format}`;
    ...
    logger.error('Erro ao exportar dados de tbfluxodifusivoinpe', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
