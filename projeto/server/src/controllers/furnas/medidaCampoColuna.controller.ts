...
import { MedidaCampoColunaModel } from "../../models/furnas/medidaCampoColuna.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: rawData, total } = await MedidaCampoColunaModel.findPaginated({
      filters: req.query,
      page,
      limit,
    });
...
export const getById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const idMedidaCampoColuna = Number(req.params.idMedidaCampoColuna);

    // ✅ MUDANÇA AQUI: Adiciona validação de NaN
    if (isNaN(idMedidaCampoColuna)) {
      res.status(400).json({
        success: false,
        error: `ID ${req.params.idMedidaCampoColuna} inválido.`,
      });
      return;
    }

    // 1. Pede o dado ao Model
    const rawData = await MedidaCampoColunaModel.findById(idMedidaCampoColuna);

    // 2. Verifica se foi encontrado
    if (!rawData) {
      res.status(404).json({
        success: false,
        error: `Registro de Medida de Campo em Coluna não encontrado.`,
      });
      return;
    }
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // ✅ MUDANÇA AQUI: Usa MedidaCampoColunaModel
    if (range === 'page') {
      const { data } = await MedidaCampoColunaModel.findPaginated({
        filters: filters || {},
        page: page || 1,
      ...
      // range === 'all'
      rawData = await MedidaCampoColunaModel.findAll({
        filters: filters || {},
      });
    }
    ...
    const fileName = `export_medida_campo_coluna_${new Date().toISOString().slice(0, 10)}.${format}`;
    ...
    logger.error('Erro ao exportar dados de tbmedidacampocoluna', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
