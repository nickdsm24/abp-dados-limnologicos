...
import { InstituicaoModel } from "../../models/furnas/instituicao.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // ✅ MUDANÇA AQUI: Usa o DataFormatterService (embora simples, mantém o padrão)
...
export const getById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.error(
      `Erro ao consultar tbinstituicao por ID ${req.params.idinstituicao}`,
      {
        message: error.message,
        stack: error.stack,
      },
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // ✅ MUDANÇA AQUI: Usa InstituicaoModel
    if (range === 'page') {
