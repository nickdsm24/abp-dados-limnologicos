//src/controllers/furnas/concentracaoGasSedimento.controller.ts
...
import { ConcentracaoGasSedimentoModel } from "../../models/furnas/concentracaoGasSedimento.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: rawData, total } = await ConcentracaoGasSedimentoModel.findPaginated({
      filters: req.query,
      page,
      limit,
    });
    ...
    logger.error(`Erro ao buscar concentrações de gás no sedimento:`, {
...
export const getById = async (req: Request, res: Response): Promise<void> => {
  try {
    const idCGS = Number(req.params.idConcentracaoGasSedimento);

    if (isNaN(idCGS)) {
      res.status(400).json({
        success: false,
        error: `ID ${req.params.idConcentracaoGasSedimento} inválido.`,
      });
      return;
    }

    // 1. Pede o dado ao Model
    const rawData = await ConcentracaoGasSedimentoModel.findById(idCGS);

    // 2. Verifica se foi encontrado
    if (!rawData) {
      res.status(404).json({
        success: false,
        error: "Registro de concentração de gás no sedimento não encontrado.",
      });
      return;
    }
    ...
    logger.error(
      `Erro ao buscar registro por ID ${req.params.idConcentracaoGasSedimento} na tabela de concentrações de gás por sedimento.`,
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // ✅ MUDANÇA AQUI: Usa ConcentracaoGasSedimentoModel
    if (range === 'page') {
      const { data } = await ConcentracaoGasSedimentoModel.findPaginated({
        filters: filters || {},
        page: page || 1,
      ...
      // range === 'all'
      rawData = await ConcentracaoGasSedimentoModel.findAll({
        filters: filters || {},
      });
    }
    ...
    const fileName = `export_concentracao_gas_sedimento_${new Date().toISOString().slice(0, 10)}.${format}`;
    ...
    logger.error('Erro ao exportar dados de tbconcentracaogassedimento', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
