// src/controllers/furnas/bioticoSuperficie.controller.ts
...
import { BioticoSuperficieModel } from '../../models/furnas/bioticoSuperficie.model';
...
  	    await BioticoSuperficieModel.findPaginated({
...
  	  const rawData = await BioticoSuperficieModel.findById(id);
...
  	} catch (error: any) {
  	  logger.error(
  	    `Erro ao consultar tbbioticosuperficie por ID ${req.params.id}`,
  	    {
  	      message: error.message,
  	      stack: error.stack,
  	    },
...
  	  if (range === 'page') {
  	    const { data } = await BioticoSuperficieModel.findPaginated({
  	      filters: filters || {},
  	      page: page || 1,
...
  	  } else {
  	    // range === 'all'
 	    rawData = await BioticoSuperficieModel.findAll({
  	      filters: filters || {},
  	    });
  	  }
...
  	  const fileName = `export_biotico_superficie_${new Date()
  	    .toISOString()
  	    .slice(0, 10)}.${format}`;
...
  	  res.send(fileBuffer);
  	} catch (error: any) {
  	  logger.error('Erro ao exportar dados de tbbioticosuperficie', {
  	    message: error.message,
  	    stack: error.stack,
  	  });
  	  res.status(500).json({
