...
import { GasesEmBolhasModel } from "../../models/furnas/gasesEmBolhas.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: rawData, total } = await GasesEmBolhasModel.findPaginated({
      filters: req.query,
      page,
      limit,
    });
...
export const getById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (isNaN(idGEBolhas)) {
      res.status(400).json({
        success: false,
        error: `ID ${req.params.idGasesEmBolhas} inválido.`,
      });
      return;
    }

    // 1. Pede o dado ao Model
    const rawData = await GasesEmBolhasModel.findById(idGEBolhas);

    // 2. Verifica se foi encontrado
    if (!rawData) {
      res.status(404).json({
        success: false,
        error: "Registro de gases em bolhas não encontrado.",
      });
      return;
    }
    ...
    logger.error(
      `Erro ao buscar registro por ID ${req.params.idGasesEmBolhas} na tabela de gases em bolhas.`,
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // ✅ MUDANÇA AQUI: Usa GasesEmBolhasModel
    if (range === 'page') {
      const { data } = await GasesEmBolhasModel.findPaginated({
        filters: filters || {},
        page: page || 1,
      ...
      // range === 'all'
      rawData = await GasesEmBolhasModel.findAll({
        filters: filters || {},
      });
    }
    ...
    const fileName = `export_gases_em_bolhas_${new Date().toISOString().slice(0, 10)}.${format}`;
    ...
    logger.error('Erro ao exportar dados de tbgasesembolhas', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
