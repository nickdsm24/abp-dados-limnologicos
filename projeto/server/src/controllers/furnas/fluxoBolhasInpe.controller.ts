...
import { FluxoBolhasInpeModel } from "../../models/furnas/fluxoBolhasInpe.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: rawData, total } = await FluxoBolhasInpeModel.findPaginated({
      filters: req.query,
      page,
      limit,
    });
    ...
    logger.error(`Erro ao buscar dados de fluxo de bolhas (INPE):`, {
...
export const getById = async (req: Request, res: Response): Promise<void> => {
  try {
    const idFBInpe = Number(req.params.idFluxoBolhasInpe);

    if (isNaN(idFBInpe)) {
      res.status(400).json({
        success: false,
        error: `ID ${req.params.idFluxoBolhasInpe} inválido.`,
      });
      return;
    }

    // 1. Pede o dado ao Model
    const rawData = await FluxoBolhasInpeModel.findById(idFBInpe);

    // 2. Verifica se foi encontrado
    if (!rawData) {
      res.status(404).json({
        success: false,
        error: "Registro de dados de fluxo de bolhas (INPE) não encontrado.",
      });
      return;
    }
    ...
    logger.error(
      `Erro ao buscar registro por ID ${req.params.idFluxoBolhasInpe} na tabela de fluxo de bolhas (INPE).`,
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // ✅ MUDANÇA AQUI: Usa FluxoBolhasInpeModel
    if (range === 'page') {
      const { data } = await FluxoBolhasInpeModel.findPaginated({
        filters: filters || {},
        page: page || 1,
      ...
      // range === 'all'
      rawData = await FluxoBolhasInpeModel.findAll({
        filters: filters || {},
      });
    }
    ...
    const fileName = `export_fluxo_bolhas_inpe_${new Date().toISOString().slice(0, 10)}.${format}`;
    ...
    logger.error('Erro ao exportar dados de tbfluxobolhasinpe', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
