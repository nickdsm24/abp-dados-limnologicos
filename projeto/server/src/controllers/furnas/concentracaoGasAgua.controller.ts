//src/controllers/furnas/concentracaoGasAgua.controller.ts
...
import { ConcentracaoGasAguaModel } from "../../models/furnas/concentracaoGasAgua.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: rawData, total } = await ConcentracaoGasAguaModel.findPaginated({
      filters: req.query,
      page,
      limit,
    });
...
export const getById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (isNaN(idCGA)) {
      res.status(400).json({
        success: false,
        error: `ID ${req.params.idConcentracaoGasAgua} inválido.`,
      });
      return;
    }

    // 1. Pede o dado ao Model
    const rawData = await ConcentracaoGasAguaModel.findById(idCGA);

    // 2. Verifica se foi encontrado
    if (!rawData) {
      res.status(404).json({
        success: false,
        error: "Registro de concentração de gás na água não encontrado.",
      });
      return;
    }
    ...
    logger.error(
      `Erro ao consultar registro por ID na tabela concentração de gás na água: ${req.params.idConcentracaoGasAgua}`,
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (range === 'page') {
      const { data } = await ConcentracaoGasAguaModel.findPaginated({
        filters: filters || {},
        page: page || 1,
      ...
      // range === 'all'
      rawData = await ConcentracaoGasAguaModel.findAll({
        filters: filters || {},
      });
    }
    ...
    const fileName = `export_concentracao_gas_agua_${new Date().toISOString().slice(0, 10)}.${format}`;
    ...
    logger.error('Erro ao exportar dados de tbconcentracaogasagua', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
...
export const getAnalytics = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const data = await ConcentracaoGasAguaModel.getAnalyticsData({
    ...
    logger.error("Erro ao gerar analytics de Concentração Gás Água", {
      message: error.message,
      stack: error.stack,
      query: req.query
    });
