...
import { MedidaCampoSuperficieModel } from "../../models/furnas/medidaCampoSuperficie.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: rawData, total } = await MedidaCampoSuperficieModel.findPaginated({
      filters: req.query,
      page,
      limit,
    });
...
export const getById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const idMedidaCampoSuperficie = Number(req.params.idMedidaCampoSuperficie);

    // ✅ MUDANÇA AQUI: Adiciona validação de NaN
    if (isNaN(idMedidaCampoSuperficie)) {
      res.status(400).json({
        success: false,
        error: `ID ${req.params.idMedidaCampoSuperficie} inválido.`,
      });
      return;
    }

    // 1. Pede o dado ao Model
    const rawData = await MedidaCampoSuperficieModel.findById(idMedidaCampoSuperficie);

    // 2. Verifica se foi encontrado
    if (!rawData) {
      res.status(404).json({
        success: false,
        error: `Registro de Medida de Campo em Superfície não encontrado.`,
      });
      return;
    }
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // ✅ MUDANÇA AQUI: Usa MedidaCampoSuperficieModel
    if (range === 'page') {
      const { data } = await MedidaCampoSuperficieModel.findPaginated({
        filters: filters || {},
        page: page || 1,
      ...
      // range === 'all'
      rawData = await MedidaCampoSuperficieModel.findAll({
        filters: filters || {},
      });
    }
    ...
    const fileName = `export_medida_campo_superficie_${new Date().toISOString().slice(0, 10)}.${format}`;
    ...
    logger.error('Erro ao exportar dados de tbmedidacamposuperficie', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
