...
// 2. Importa o Model
import { PfqModel } from "../../models/furnas/pfq.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const { data: rawData, total } = await PfqModel.findPaginated({
...
export const getById = async (req: Request, res: Response): Promise<void> => {
    try {
        const idPfq = Number(req.params.id);
        ...
        const rawData = await PfqModel.findById(idPfq);

        // 2. Verifica se foi encontrado
        if (!rawData) {
            res.status(404).json({
                success: false,
                error: `Registro PFQ não encontrado.`,
            });
            return;
        }

        // 3. Retorna os dados crus (sem formatação de detalhe, conforme exemplo)
        const data = rawData;
        ...
        logger.error(`Erro ao consultar tbpfq por ID ${req.params.id}`, {
            message: error.message,
            stack: error.stack,
        });
        res.status(500).json({
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (range === "page") {
            const { data } = await PfqModel.findPaginated({
                filters: filters || {},
                page: page || 1,
            ...
            // range === 'all'
            rawData = await PfqModel.findAll({
                filters: filters || {},
            });
        }
        ...
        const fileName = `export_pfq_${new Date().toISOString().slice(0, 10)}.${format}`;
        ...
        logger.error("Erro ao exportar dados de tbpfq", {
            message: error.message,
            stack: error.stack,
        });
        res.status(500).json({
