...
import { ParametrosBiologicosFisicosAguaModel } from "../../models/furnas/parametrosBiologicosFisicosAgua.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: rawData, total } = await ParametrosBiologicosFisicosAguaModel.findPaginated({
      filters: req.query,
      page,
      limit,
    });
...
export const getById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const idParametros = Number(req.params.idParametrosBiologicosFisicosAgua);

    // ✅ MUDANÇA AQUI: Adiciona validação de NaN
    if (isNaN(idParametros)) {
      res.status(400).json({
        success: false,
        error: `ID ${req.params.idParametrosBiologicosFisicosAgua} inválido.`,
      });
      return;
    }

    // 1. Pede o dado ao Model
    const rawData = await ParametrosBiologicosFisicosAguaModel.findById(idParametros);

    // 2. Verifica se foi encontrado
    if (!rawData) {
      res.status(404).json({
        success: false,
        error: `Registro de Parâmetros Biológicos e Físicos da Água não encontrado.`,
      });
      return;
    }
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // ✅ MUDANÇA AQUI: Usa ParametrosBiologicosFisicosAguaModel
    if (range === 'page') {
      const { data } = await ParametrosBiologicosFisicosAguaModel.findPaginated({
        filters: filters || {},
        page: page || 1,
      ...
      // range === 'all'
      rawData = await ParametrosBiologicosFisicosAguaModel.findAll({
        filters: filters || {},
      });
    }
    ...
    const fileName = `export_params_bio_fis_agua_${new Date().toISOString().slice(0, 10)}.${format}`;
    ...
    logger.error('Erro ao exportar dados de tbparametrosbiologicosfisicosagua', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
