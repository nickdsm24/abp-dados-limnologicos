...
// 2. Importa o Model
import { ReservatorioModel } from "../../models/balcar/reservatorio.model";
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const { data: rawData, total } = await ReservatorioModel.findPaginated({
        ...
        //    (Mantemos o padrão de formatação mesmo sem joins)
        ...
        logger.error("Erro ao buscar reservatórios", {
...
export const getById = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const rawData = await ReservatorioModel.findById(idreservatorio);
        ...
        logger.error("Erro ao buscar reservatório por ID", {
            message: error.message,
            stack: error.stack,
        });
        res.status(500).json({
            success: false,
            error: "Erro ao buscar reservatório.",
        });
    }
};
...
export const exportData = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (range === "page") {
            const { data } = await ReservatorioModel.findPaginated({
                filters: filters || {},
                page: page || 1,
            ...
            // range === 'all'
            rawData = await ReservatorioModel.findAll({
                filters: filters || {},
            });
        }
        ...
        const fileName = `export_reservatorio_${new Date().toISOString().slice(0, 10)}.${format}`;
        ...
        logger.error("Erro ao exportar dados de tbreservatorio", {
            message: error.message,
            stack: error.stack,
        });
        res.status(500).json({
