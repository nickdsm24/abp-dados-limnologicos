 ...
 */
const buildEstacaoQuery = (filters: any) => {
    ...
    const mainQuery = `${baseQuery} ${whereString} ORDER BY a.rotulo`;
...
/**
 * Classe Model para encapsular o acesso a dados da tbestacao.
 */
export class EstacaoModel {
    ...
    public static async findAll(options: { filters: any }): Promise<any[]> {
        ...
        const { mainQuery, values } = buildEstacaoQuery(filters);
    ...
    /**
     * Busca um único registro pelo ID (tratado como string, conforme controller original).
     */
