 ...
 */
const fluxoBolhasInpeColumnMap = {
  // Chaves de Range (type: 'number' ou 'date' no frontend)
  idFluxoBolhasInpe: 'a.idfluxobolhasinpe',
 ...
 */
const buildFluxoBolhasInpeQuery = (filters: any) => {
  // Query base para selecionar os dados (para listagem)
  // Baseada na query 'getAll' do controller antigo
  const baseQuery = `
    SELECT
        a.idFluxoBolhasInpe, a.dataMedida, a.horaMedida,
        a.profundidade, a.ch4, a.ch4_desviopadrao, a.ch4_amostras,
        b.idsitio, b.nome AS sitio_nome,
        c.idcampanha, c.nroCampanha
    FROM tbfluxobolhasinpe a
  ...
  const countQuery = `
    SELECT COUNT(a.idfluxobolhasinpe)
    FROM tbfluxobolhasinpe a
    LEFT JOIN tbsitio b ON a.idSitio = b.idSitio
    LEFT JOIN tbcampanha c ON a.idCampanha = c.idCampanha
  `;
...
/**
 * Classe Model para encapsular o acesso a dados da tbfluxobolhasinpe.
 */
export class FluxoBolhasInpeModel {
  /**
  ...
  public static async findPaginated(options: {
    ...
    const { mainQuery, countText, values, paramIndex } =
      buildFluxoBolhasInpeQuery(filters);
  ...
  public static async findAll(options: { filters: any }): Promise<any[]> {
    ...
    const { mainQuery, values } = buildFluxoBolhasInpeQuery(filters);
