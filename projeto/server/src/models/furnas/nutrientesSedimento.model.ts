 ...
 */
const nutrientesSedimentoColumnMap = {
  // Chaves de Range (type: 'number' ou 'date' no frontend)
  idNutrientesSedimento: 'a.idnutrientessedimento',
 ...
 */
const buildNutrientesSedimentoQuery = (filters: any) => {
  // Query base para selecionar os dados (para listagem)
  // Baseada na query 'getAll' do controller antigo
  const baseQuery = `
    SELECT 
        a.idNutrientesSedimento, a.dataMedida, a.horaMedida, a.profundidade,
        a.batimetria, a.n2, a.pt, a.tc,
  ...
  const countQuery = `
    SELECT COUNT(a.idnutrientessedimento)
    FROM tbnutrientessedimento AS a
    LEFT JOIN tbcampanha AS b ON a.idCampanha = b.idCampanha
    LEFT JOIN tbsitio AS c ON a.idSitio = c.idSitio
  `;
...
/**
 * Classe Model para encapsular o acesso a dados da tbnutrientessedimento.
 */
export class NutrientesSedimentoModel {
  /**
  ...
  public static async findPaginated(options: {
    ...
    const { mainQuery, countText, values, paramIndex } =
      buildNutrientesSedimentoQuery(filters);
  ...
  public static async findAll(options: { filters: any }): Promise<any[]> {
    ...
    const { mainQuery, values } = buildNutrientesSedimentoQuery(filters);
  ...
  public static async findById(id: number): Promise<any | null> {
    ...
    const result = await furnasPool.query(
      `
      SELECT 
          a.*, -- Todos os campos de tbnutrientessedimento
          b.idcampanha, b.nrocampanha,
