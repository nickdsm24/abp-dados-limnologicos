 ...
 */
const medidaCampoSuperficieColumnMap = {
  // Chaves de Range (type: 'number' ou 'date' no frontend)
  idMedidaCampoSuperficie: 'a.idmedidacamposuperficie',
 ...
 */
const buildMedidaCampoSuperficieQuery = (filters: any) => {
  // Query base para selecionar os dados (para listagem)
  // Baseada na query 'getAll' do controller antigo
  const baseQuery = `
    SELECT 
        a.idMedidaCampoSuperficie, a.dataMedida, a.horaMedida, a.secchi,
        a.tempagua, a.condutividade, a._do, a.ph, a.turbidez,
  ...
  const countQuery = `
    SELECT COUNT(a.idmedidacamposuperficie)
    FROM tbmedidacamposuperficie AS a
    LEFT JOIN tbcampanha AS b ON a.idCampanha = b.idCampanha
    LEFT JOIN tbsitio AS c ON a.idSitio = c.idSitio
  `;
  ...
  const { whereClause, params, nextIndex } = FilterService.buildFilter(
    filters,
    medidaCampoSuperficieColumnMap,
...
/**
 * Classe Model para encapsular o acesso a dados da tbmedidacamposuperficie.
 */
export class MedidaCampoSuperficieModel {
  /**
  ...
  public static async findPaginated(options: {
    ...
    const { mainQuery, countText, values, paramIndex } =
      buildMedidaCampoSuperficieQuery(filters);
  ...
  public static async findAll(options: { filters: any }): Promise<any[]> {
    ...
    const { mainQuery, values } = buildMedidaCampoSuperficieQuery(filters);
  ...
  public static async findById(id: number): Promise<any | null> {
    ...
    const result = await furnasPool.query(
      `
      SELECT 
          a.*, -- Todos os campos de tbmedidacamposuperficie
          b.idcampanha, b.nrocampanha,
