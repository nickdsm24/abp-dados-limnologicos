 ...
 */
const dadosPrecipitacaoColumnMap = {
  // Chaves de Range (type: 'number' ou 'date' no frontend)
  idDadosPrecipitacao: 'a.iddadosprecipitacao',
 ...
 */
const buildDadosPrecipitacaoQuery = (filters: any) => {
  ...
  const countQuery = `
    SELECT COUNT(a.iddadosprecipitacao)
    FROM tbdadosprecipitacao a
    LEFT JOIN tbreservatorio b ON a.idReservatorio = b.idReservatorio
  `;
  ...
  const mainQuery = `${baseQuery} ${whereString} ORDER BY a.dataMedida DESC, a.idDadosPrecipitacao DESC`;
...
/**
 * Classe Model para encapsular o acesso a dados da tbdadosprecipitacao.
 */
export class DadosPrecipitacaoModel {
  /**
  ...
  public static async findPaginated(options: {
    ...
    const { mainQuery, countText, values, paramIndex } =
      buildDadosPrecipitacaoQuery(filters);
  ...
  public static async findAll(options: { filters: any }): Promise<any[]> {
    ...
    const { mainQuery, values } = buildDadosPrecipitacaoQuery(filters);
