 ...
 */
const fluxoDifusivoInpeColumnMap = {
  // Chaves de Range (type: 'number' ou 'date' no frontend)
  idFluxoDifusivoInpe: 'a.idfluxodifusivoinpe',
  dataMedida: 'a.datamedida',
  profundidade: 'a.profundidade',
  co2: 'a.co2',
  co2_desviopadrao: 'a.co2_desviopadrao',
  co2_amostras: 'a.co2_amostras',
  ch4: 'a.ch4',
  ch4_desviopadrao: 'a.ch4_desviopadrao',
  ch4_amostras: 'a.ch4_amostras',
 ...
 */
const buildFluxoDifusivoInpeQuery = (filters: any) => {
  // Query base para selecionar os dados (para listagem)
  // Baseada na query 'getAll' do controller antigo
  // Incluímos as colunas _amostras que estavam faltando na query original
  const baseQuery = `
    SELECT
        a.idFluxoDifusivoInpe, a.dataMedida, a.horaMedida,
        a.profundidade,
        a.co2, a.co2_desviopadrao, a.co2_amostras,
        a.ch4, a.ch4_desviopadrao, a.ch4_amostras,
        b.idsitio, b.nome AS sitio_nome,
        c.idcampanha, c.nroCampanha
    FROM tbfluxodifusivoinpe a
  ...
  const countQuery = `
    SELECT COUNT(a.idfluxodifusivoinpe)
    FROM tbfluxodifusivoinpe a
    LEFT JOIN tbsitio b ON a.idSitio = b.idSitio
    LEFT JOIN tbcampanha c ON a.idCampanha = c.idCampanha
  `;
  ...
  const mainQuery = `${baseQuery} ${whereString} ORDER BY a.dataMedida DESC, a.horaMedida DESC, a.profundidade ASC`;
...
/**
 * Classe Model para encapsular o acesso a dados da tbfluxodifusivoinpe.
 */
export class FluxoDifusivoInpeModel {
  /**
  ...
  public static async findPaginated(options: {
    ...
    const { mainQuery, countText, values, paramIndex } =
      buildFluxoDifusivoInpeQuery(filters);
  ...
  public static async findAll(options: { filters: any }): Promise<any[]> {
    ...
    const { mainQuery, values } = buildFluxoDifusivoInpeQuery(filters);
