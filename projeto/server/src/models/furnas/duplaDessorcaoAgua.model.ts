 ...
 */
const duplaDessorcaoAguaColumnMap = {
  // Chaves de Range (type: 'number' ou 'date' no frontend)
  idDuplaDessorcaoAgua: 'a.iddupladessorcaoagua',
 ...
 */
const buildDuplaDessorcaoAguaQuery = (filters: any) => {
  // Query base para selecionar os dados (para listagem)
  // Baseada na query 'getAll' do controller antigo
  // Adicionamos idsitio e idcampanha para o DataFormatterService
  const baseQuery = `
    SELECT
        a.idDuplaDessorcaoAgua, a.dataMedida, a.horaMedida,
        a.profundidade, a.co2, a.o2, a.n2, a.ch4, a.n2o,
  ...
  const countQuery = `
    SELECT COUNT(a.iddupladessorcaoagua)
    FROM tbdupladessorcaoagua a
    LEFT JOIN tbsitio b ON a.idSitio = b.idSitio
    LEFT JOIN tbcampanha c ON a.idCampanha = c.idCampanha
  `;
...
/**
 * Classe Model para encapsular o acesso a dados da tbdupladessorcaoagua.
 */
export class DuplaDessorcaoAguaModel {
  /**
  ...
  public static async findPaginated(options: {
    ...
    const { mainQuery, countText, values, paramIndex } =
      buildDuplaDessorcaoAguaQuery(filters);
  ...
  public static async findAll(options: { filters: any }): Promise<any[]> {
    ...
    const { mainQuery, values } = buildDuplaDessorcaoAguaQuery(filters);
