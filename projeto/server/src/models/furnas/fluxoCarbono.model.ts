 ...
 */
const fluxoCarbonoColumnMap = {
  // Chaves de Range (type: 'number' ou 'date' no frontend)
  idFluxoCarbono: 'a.idfluxocarbono',
 ...
 */
const buildFluxoCarbonoQuery = (filters: any) => {
  ...
  const baseQuery = `
    SELECT
        a.idFluxoCarbono, a.dataMedida, a.horaMedida,
        a.producaofitoplanctonica, a.carbonoorganicoexcretado,
        a.respiracaofito, a.producaobacteriana,
        a.respiracaobacteriana, a.taxasedimentacao,
...
/**
 * Classe Model para encapsular o acesso a dados da tbfluxocarbono.
 */
export class FluxoCarbonoModel {
  /**
  ...
  public static async findPaginated(options: {
    ...
    const { mainQuery, countText, values, paramIndex } =
      buildFluxoCarbonoQuery(filters);
  ...
  public static async findAll(options: { filters: any }): Promise<any[]> {
    ...
    const { mainQuery, values } = buildFluxoCarbonoQuery(filters);
