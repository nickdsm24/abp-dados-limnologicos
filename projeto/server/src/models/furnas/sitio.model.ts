...
const sitioColumnMap = {
    ...
    // O frontend pode enviar { nome: 'Ponto 1' } ou { reservatorio: 'SERRA DA MESA' }
    nome: 'a.nome', 
...
const buildSitioQuery = (filters: any) => {
    ...
    // Inclui aliases (sitio_nome, reservatorio_nome) para o DataFormatterService
    const baseQuery = `
        SELECT 
            a.idsitio,
...
/**
 * Classe Model para encapsular o acesso a dados da tbsitio.
 */
export class SitioModel {
     ...
     * (Segue o padrão do abiotico.model.ts, selecionando a.* e campos com alias)
     */
    public static async findById(id: number): Promise<any | null> {
        const result = await furnasPool.query(
            ...
            SELECT 
                ...
                b.idreservatorio AS reservatorio_id, -- ID aninhado
                b.nome AS reservatorio_nome,
                b.lat AS reservatorio_lat,
                b.lng AS reservatorio_lng
            FROM tbsitio AS a
