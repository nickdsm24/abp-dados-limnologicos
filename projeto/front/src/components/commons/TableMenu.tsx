...
export function Menu({ title, tabelas, tabelaAtiva, onSelectTabela, database }: MenuProps) {
  ...
  const databaseDisplayName = useMemo(() => {
    switch (database) {
      case 'furnas': return 'Furnas';
      case 'balcar': return 'Balcar';
      case 'sima': return 'Sima';
  ...
  return (
    ...
    <aside className="w-64 bg-[#2d3748] text-white flex flex-col">
        ...
        {tabelas.map((tabela) => {
          ...
          return (
            <button
              ...
              `}
            >
              {/* 7. Ícone 'Database' trocado por 'Table2' */}
              <Table2 className="h-5 w-5 mr-3" />
      ...
      <div className="p-4 border-t border-white/20 text-center text-xs text-gray-300">
        <p>&copy; {new Date().getFullYear()} - Projeto {databaseDisplayName}</p>
      </div>
    </aside>
  );
}
