...
// --- Sidebar (MODIFICADA PARA RESPONSIVIDADE) ---
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, closeSidebar }) => {
  ...
  const menuItems: MenuItem[] = [
    ...
    { label: "Informações do Projeto", path: "/balcar-info", icon: InfoIcon },
  ];
  ...
  return (
    <>
      ...
      {/* Container Sidebar 
          Correção Desktop: Adicionado 'md:translate-x-0' para garantir visibilidade 
          independente do estado 'isOpen' em telas médias e maiores.
      */}
      <aside
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Link
                to="/furnas"
                onClick={closeSidebar}
                className="group relative aspect-square rounded-lg overflow-hidden shadow-lg cursor-pointer transition-all duration-300 hover:scale-[1.03]"
              >
                <img
                  src="/furnas.jpg"
                  alt="Projeto Furnas"
              ...
              <Link
                to="/sima"
                onClick={closeSidebar}
                className="group relative aspect-square rounded-lg overflow-hidden shadow-lg cursor-pointer transition-all duration-300 hover:scale-[1.03] bg-gray-700"
              >
                <img
                  src="/sima.png"
                  alt="Projeto Sima"
                  className="h-1/2 w-auto object-contain absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 group-hover:opacity-80 transition-opacity"
                />
                <div className="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition-all"></div>
                <span className="absolute bottom-2 left-2 text-sm font-bold text-white drop-shadow-md">
                  Sima
...
// --- Layout Principal (COM ESTADO DE SIDEBAR) ---
        ...
        <div className="md:hidden bg-white p-4 shadow-sm flex items-center justify-between z-20">
          <div className="flex items-center gap-3">
            <button
              ...
              className="text-gray-600 hover:text-[#006666] transition-colors p-1"
